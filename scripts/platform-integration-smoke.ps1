...
try {
    ...
    Assert-Condition ($completedTrainingJob.result_payload.model_artifact.candidate_id.Length -gt 0) "Retraining prep job model artifact candidate_id is missing."
    ...
    $summary.jobs.retraining_job = [ordered]@{
        ...
        candidate_id = $completedTrainingJob.result_payload.model_artifact.candidate_id
        artifact_version = $completedTrainingJob.result_payload.model_artifact.artifact_version
    ...
    $matchingRegistryEntry = $registryEntry | Where-Object { $_.candidate_id -eq $completedTrainingJob.result_payload.model_artifact.candidate_id } | Select-Object -First 1
    ...
    $registryHistory = Invoke-JsonRequest -Method Get -Uri "$baseUrl/api/model-registry/$($completedTrainingJob.result_payload.model_artifact.candidate_id)/history?limit=10" -Headers $authHeaders
    ...
    $summary.jobs.registry = [ordered]@{
        candidate_id = $matchingRegistryEntry.candidate_id
        manifest_id = $matchingRegistryEntry.manifest_id
        artifact_version = $matchingRegistryEntry.artifact_version
